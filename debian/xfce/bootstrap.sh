...
proot-distro login debian-xfce --user debian-xfce' >> /data/data/com.termux/files/usr/bin/startdebian-xfce

# for X11 session
cat <<EOF > /data/data/com.termux/files/usr/bin/startdebian-xfce-x11
#!/bin/sh
LD_PRELOAD=/system/lib64/libskcodec.so
pulseaudio --start --load="module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1" --exit-idle-time=-1
...
chmod +x /data/data/com.termux/files/usr/bin/startdebian-xfce*
...
echo 'To start command line session: startdebian-xfce'
echo 'To start X11 session: startdebian-xfce-x11'
