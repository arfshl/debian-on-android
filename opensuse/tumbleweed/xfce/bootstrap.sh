...
proot-distro login tumbleweed-xfce --user opensuse-xfce' >> /data/data/com.termux/files/usr/bin/tumbleweed-xfce

# for X11 session
cat <<'EOF' > /data/data/com.termux/files/usr/bin/tumbleweed-xfce-x11
#!/bin/sh
LD_PRELOAD=/system/lib64/libskcodec.so
pulseaudio --start --load="module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1" --exit-idle-time=-1
...
proot-distro login tumbleweed-xfce --shared-tmp -- /bin/sh -c 'kill -9 $(pgrep -f "x11") 2>/dev/null'
virgl_test_server_android &
termux-x11 :0 >/dev/null &
sleep 3
proot-distro login tumbleweed-xfce --shared-tmp -- /bin/sh -c 'export PULSE_SERVER=127.0.0.1 && export XDG_RUNTIME_DIR=${TMPDIR} && su - opensuse-xfce -c "DISPLAY=:0 GALLIUM_DRIVER=virpipe dbus-launch --exit-with-session startxfce4"'
EOF

# Make all of them executable
chmod +x /data/data/com.termux/files/usr/bin/tumbleweed-xfce*

# Install rootfs under aliases
proot-distro install opensuse --override-alias tumbleweed-xfce
